...
class BaseDAOSSE(Generic[State, PhyState, ObsState, Param, Config]):
   ...
   def __init__(self, nature: Model[State, PhyState, Param, Config], model: Model[State, PhyState, Param, Config], 
               observation: Observation, scheme: Scheme[PhyState, ObsState], diagnostics: Diagnostics, config: DAConfig):
      self.cse = False
      self.nature = nature
      self.model = model
   ...
   def verify(self, dastate: DAState, law0: Param, param0: Param) -> DAScore:
      ...
      if self.cse:
         phystatef = self.nature.mod2phy(dastate.forecast, law0)
         obsstatef = self.observation.Hforward(phystatef)
      else:
      ...
      valid = self.observation.get_valid_mask(obstruth, obsstatef, obsstatea)
      obstruth = self.observation.mask_invalid_obs(obstruth, valid, jnp.nan)
      obsstatef = self.observation.mask_invalid_obs(obsstatef, valid, jnp.nan)
      obsstatea = self.observation.mask_invalid_obs(obsstatea, valid, jnp.nan)
   ...
   def cycle(self, key: jax.Array, true0: State, state0: State, law0: Param, param0: Param, ncycle: int, save_freq: Optional[int] = None) -> Tuple[jax.Array, DAState, DAScore]:
      ...
      if save_freq is None:
         ...
         def inner_loop(carry, _):
            ...
            if self.cse:
               phystatef = self.nature.mod2phy(state.forecast, law0)
            else:
...
class DAOSSE(BaseDAOSSE[State, PhyState, ObsState, Param, Config]):
   ...
   def forward(self, key: jax.Array, truth0: State, state0: State, law0: Param, param0: Param) -> tuple[jax.Array, DAState]:
      ...
      phystate1f = self.ensemble.mod2phy(state1f, param0)
      phystate1a = self.scheme.forward(phystate1f, obsstate, obs, obserr)
      state1a = self.ensemble.phy2mod(phystate1a-phystate1f, state1f, param0)
...
class IDAOSSE(BaseDAOSSE[State, PhyState, ObsState, Param, Config]):
   ...
   def forward(self, key: jax.Array, truth0: State, state0: State, law0: Param, param0: Param) -> tuple[jax.Array, DAState]:
      ...
      phytruth = self.nature.mod2phy(trajectory, law0)
      obstruth = self.observation.Hforward(phytruth)
      # Map model space to observation space
      state1f, trajectory = self.ensemble.integrate(state0, param0, self.config.dawindow, self.config.obsfreq)
      phystate = self.ensemble.mod2phy(trajectory, param0)
      obsstate = self.ensemble.Hforward(phystate)
      
      # Compute combined validity mask: valid only where BOTH truth AND all members are valid
      obsstatem = self.scheme.mean(obsstate)
      valid = self.observation.get_valid_mask(obstruth, obsstatem)
      # Filter ALL observation-space quantities using the combined mask
      obstruth = self.observation.mask_invalid_obs(obstruth, valid, jnp.nan)
      obsstate = self.observation.mask_invalid_obs(obsstate, valid, 0.0)
      ...
      phystate1f = self.ensemble.mod2phy(state1f, param0)
      phystate1a = self.scheme.forward(phystate1f, obsstate2, obs, obserr)
      state1a = self.ensemble.phy2mod(phystate1a-phystate1f, state1f, param0)
...
class IDACSE(BaseDAOSSE[State, PhyState, ObsState, Param, Config]):
   ...
   def forward(self, key: jax.Array, truth0: State, state0: State, law0: Param, param0: Param) -> tuple[jax.Array, DAState]:
      ...
      mean1a = self.scheme.mean(phystate1a)
      mean1f = self.scheme.mean(truth1)
      mean1a = self.nature.phy2mod(mean1a-mean1f, truth1, law0)
      ...
      phytruth1 = self.nature.mod2phy(truth1, param0)
      phystate1a = self.ensemble.mod2phy(state1a, param0)
      phytruth2, phystate2a = self.ctlscheme.ctlforward(phytruth1, phystate1a, obsstate2, obs, obserr)
      truth2 = self.nature.phy2mod(phytruth2-phytruth1, truth1, param0)
      state2a = self.ensemble.phy2mod(phystate2a-phystate1a, state1a, param0)
      ...
      #truth1_ensemble = jax.tree_util.tree_map(
         ...
         #truth1)
      return key, DAState(truth2, truth1, state2a)
